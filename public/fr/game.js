...
function azioneGiocatore(p, testo) {
  ...
  if (n > 0 && /d[oò]|dare|pago|pagare|offro|regal|prest|restitu|rendo|donne|paie|offre|prête|rends|rembourse/.test(t)) {
    ...
    else { const m = -trasferisci(p, -n, 'dal giocatore'); if (p.debito) { const r = Math.min(p.debito, m); p.debito -= r; if (!p.debito) { p.aff = clamp(p.aff + 6); ricorda(p, 'm\'a rendu l\'argent, personne sérieuse'); } else ricorda(p, `mi ha restituito ${Ar(r)} Ar, ne mancano ${Ar(p.debito)}`); } else if (p.richiesta && m >= p.richiesta) { p.richiesta = 0; p.aff = clamp(p.aff + 4); ricorda(p, `mi ha dato i ${Ar(m)} Ar che avevo chiesto`); } else { p.aff = clamp(p.aff + Math.min(8, Math.round(m / 5000))); p.favoriRicevuti = (p.favoriRicevuti || 0) + 1; ricorda(p, `mi ha dato ${Ar(m)} Ar`); }
      nota = `HA DAVVERO CONSEGNATO ${Ar(m)} Ar: ora li hai in tasca`; eff.push(`💸 Hai dato ${Ar(m)} Ar à ${p.nome}.`); S.eco.spesaGiocatore += m; }
  }
  ...
  else if (/rub|frego|scipp|vole|pique/.test(t)) { if (Math.random() < 0.4 && p.eco && p.eco.soldi > 0) { const m = Math.min(p.eco.soldi, rnd(2000, 20000)); trasferisci(p, m, 'furto subito'); nota = `GLI HAI RUBATO ${Ar(m)} Ar senza che se ne accorgesse (lui/lei NON lo sa)`; eff.push(`🕵️ Hai rubato ${Ar(m)} Ar à ${p.nome}.`); } else { p.aff = clamp(p.aff - 40); ricorda(p, 'HA PROVATO A DERUBARMI'); S.fedina = (S.fedina || 0) + 1; nota = 'TI HA BECCATO MENTRE PROVAVA A DERUBARTI'; eff.push(`🚔 Beccato/a a rubare a ${p.nome}: affinità -40, fedina sporca.`); spargiVoce(p, 'è un ladro/una ladra'); } }

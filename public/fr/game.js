...
function sistemaPrompt(p) {
  ...
  return `Tu es ${p.nome} ${p.cognome}, ${p.eta} ans, ${p.gen === 'M' ? 'homme' : 'femme'}, tu vis à ${p.quartiere}, Antananarivo (Madagascar). Rôle envers le joueur : ${ruoloTxt(p)}${p.stato !== 'conoscente' ? ' et ' + STATO_LBL[p.stato] : ''}. Caractère : ${trattiTxt(p)}. Affinité avec le joueur : ${p.aff}/100${p.rom ? ', attirance ' + p.rom + '/100' : ''}. Services que tu lui as déjà rendus : ${(p.favori || 0)} ; services qu'il/elle t'a rendus : ${(p.favoriRicevuti || 0)}${p.debito ? `; il/elle te doit encore ${Ar(p.debito)} Ar` : ''}.
...
function applicaAzioniPNG(p, out) {
  ...
  for (const [k, v] of tags) {
    ...
    if (k === 'OSPITA' && p.aff >= 40 && hasCasaPNG) { const gg = Math.min(14, Math.max(1, n(v) || 3)); S.ospite = { id: p.id, nome: p.nome, gg, q: QUARTIERI.find(q => q.nome === p.quartiere)?.id || S.q }; p.favori = (p.favori || 0) + 1; ricorda(p, `je l'ai hébergé(e) pendant ${gg} nuits`); eff.push(`🛏️ ${p.nome} t'héberge pendant ${gg} nuits à ${p.quartiere}: vai lì e usa "Dormi da ${p.nome}".`); }

...
/* ---------- AZIONI ---------- */
const A = {
  pronto(negozio, i) { const c = PRONTO[negozio][i]; const pr = prezzoPronto(c); if (c.mattina && ora() > 10) return toast('Les mofo gasy ne se vendent que le matin.'); if (!paga(pr)) return toast('Tu n\'as pas assez d\'argent.'); S.bis.fame = clamp(S.bis.fame + c.fame); S.bis.umore = clamp(S.bis.umore + (c.umore || 3)); S.bis.salute = clamp(S.bis.salute + (c.salute || 0)); S.stat.pasti++; avanza(20); log(`Tu as mangé ${c.nome} (${Ar(pr)} Ar).`); if (c.rischio && Math.random() < c.rischio && !S.malattie.some(m => m.tipo === 'diarrea')) { S.malattie.push({ tipo: 'diarrea', gg: 3 }); log(`La nourriture n'était pas fraîche… Diarrhée. MALUS : ${malusMalattia('diarrea')}`, 'bad'); } },
  ...
  cucina(rid) {
    const r = RICETTE.find(x => x.id === rid); if (!haCasa()) return toast('Il faut un logement pour cuisiner.'); if (S.loc !== 'casa') return toast('Tu dois être chez toi.');
    ...
    log(`Tu as cuisiné ${r.nome}${fuoco ? ' (' + NOMI_DISP[fuoco].split(' ')[0] + ')' : ''} : faim +${r.fame}.`);
  },
...
function vAgenzia() {
  ...
  ['Locations', 'Ventes', 'Terrains'].forEach(sez => { const its = lista.filter(c => sez === 'Locations' ? c.affitto : sez === 'Ventes' ? c.prezzo && c.tipo !== 'terreno' : c.tipo === 'terreno'); if (!its.length) return; h += `<h3 style="margin-top:8px">${sez}</h3>`; its.forEach(c => h += act(c.nome, () => conferma('🏘️ ' + (c.tipo === 'terreno' ? 'Acheter le terrain ?' : c.prezzo ? 'Acheter la maison ?' : 'Louer ?'), `<b>${c.nome}</b><br>${c.desc || ''}<br>${c.tipo === 'terreno' ? '💰 ' + Ar(c.prezzo) + ' Ar' : c.prezzo ? '💰 ' + Ar(c.prezzo) + ' Ar en une seule fois · confort ' + c.comfort : '💰 caution ' + Ar(c.affitto * 2) + ' Ar tout de suite, puis ' + Ar(c.affitto) + ' Ar chaque 1er du mois · confort ' + c.comfort}${haCasa() && c.tipo !== 'terreno' ? '<br><b>Tu quittes ton logement actuel (' + casa().nome + ').</b>' : ''}`, () => A.affitta(c.id), c.tipo === 'terreno' ? 'Acheter' : c.prezzo ? 'Acheter' : 'Louer'), c.tipo === 'terreno' ? `${Ar(c.prezzo)} Ar` : c.prezzo ? `ACHAT ${Ar(c.prezzo)} Ar · confort ${c.comfort}` : `${Ar(c.affitto)} Ar/mois (caution ${Ar(c.affitto * 2)}) · confort ${c.comfort}`)); });
...
function renderModal() {
  ...
  if (M.tipo === 'vai') {
    ...
    e.innerHTML = `<div class="box"><h3>📍 ${q.nome}</h3><p class="mut">${q.desc}</p><p class="mut">${Q(S.q).nome} → ${q.nome} · ${distKm(S.q, M.id)} km · heure ${oraStr()}${inTraffico('taxibe') ? ' · 🚦 heure de pointe : transports +70%' : ''}</p>${extra}${riga('🚶', 'À pied', 'piedi')}${riga('🚌', 'Taxi-be', 'taxibe')}${riga('🚕', 'Taxi', 'taxi')}${v ? riga(v.icon, v.nome, 'proprio') : ''}<p class="mut" style="margin-top:8px">Lieux : ${q.poi.map(p => POI[p].icon + ' ' + POI[p].nome.split(' (')[0]).join(' · ')}</p></div>`;
    return;
  }
  if (M.tipo === 'bucato') {

...
const TRATTI_DESC = { allegro: 'tu ris facilement, tu plaisantes, tu vois le bon côté', sospettoso: 'tu te méfies, tu poses des questions, tu gardes tes distances tant que tu n\'es pas convaincu', chiacchierone: 'tu parles beaucoup, tu racontes ta vie et celle du quartier, tu as du mal à conclure', timido: 'phrases courtes, tu es gêné, tu t\'ouvres petit à petit', severo: 'tu es direct et rigide, tu exiges respect et ponctualité', generoso: 'tu offres et tu aides volontiers même si tu as peu', pettegolo: 'tu sais tout sur tout le monde et tu le racontes ; tu aimes découvrir des choses sur le joueur', religioso: 'tu cites Dieu et l\'église, tu juges les vices et l\'alcool, tu es accueillant', ambizioso: 'tu parles d\'argent, de projets, de ceux qui comptent ; tu respectes ceux qui se bougent', pigro: 'tu minimises, tu remets à plus tard, tu n\'as pas envie de faire d\'efforts', romantico: 'tu es sentimental, tu fais des compliments, tu tombes amoureux facilement', pratico: 'tu vas droit au but, tu parles de prix, de solutions et de choses concrètes', ironico: 'tu taquines avec affection, tu es sarcastique', ansioso: 'tu t\'inquiètes de tout, tu demandes des confirmations', orgoglioso: 'tu n\'admets pas tes torts, tu te vexes facilement', materno: 'tu prends soin, tu demandes s\'il/elle a mangé, tu grondes avec affection', furbo: 'tu cherches ton intérêt, tu négocies, tu promets à moitié', gentile: 'tu es courtois et patient, tu n\'élèves jamais la voix' };
const FIGURE_FISSE = { mercato: ['vendeuse de riz au marché', 'vendeur de brèdes et de tomates'], epicerie: ['propriétaire de l\'épicerie'], minimarket: ['caissière du mini market'], super: ['caissière du supermarché'], bagni: ['gardien des toilettes publiques'], gargote: ['cuisinière patronne de l\'hotely'], ristorante: ['serveur du restaurant'], annunci: ['employée de l\'agence pour l\'emploi'], agenzia: ['agent immobilier'], farmacia: ['pharmacien(ne)'], auto: ['vendeur de voitures d\'occasion'], stadio: ['entraîneur de l\'équipe du quartier'], palestra: ['coach de la salle de sport'], chiesa: ['pasteur de la FJKM'], bar: ['barman'], lago: ['vendeur de glaces au lac'], zonafranca: ['gardien de la zone franche'], aeroporto: ['chauffeur de taxi de l\'aéroport'], fornace: ['chef de la briqueterie'], grossista: ['grossiste en riz'], fabbrica: ['gardien de l\'usine'], rova: ['guide touristique du Rova'], comune: ['employé de la Commune'] };
const FIGURE_OCC = { negozio: ['client dans la file', 'maman avec un enfant qui fait ses courses', 'garçon envoyé faire des courses', 'dame qui marchande le prix'], cibo: ['client à la table d\'à côté', 'étudiant qui mange vite', 'ouvrier en pause déjeuner', 'chauffeur de taxi qui boit un café'], strada: ['passant', 'vendeur ambulant de mofo gasy', 'chauffeur de taxi qui attend des clients', 'tireur de pousse-pousse', 'étudiante qui attend le taxi-be', 'grand-mère qui vend des arachides', 'maçon qui rentre du chantier', 'garçon qui vend des cartes téléphoniques'], ufficio: ['personne qui attend au guichet', 'jeune qui remplit un formulaire'], salute: ['patient en salle d\'attente', 'infirmière de garde'], svago: ['garçon qui joue au ballon', 'couple qui se promène', 'touriste vazaha avec son guide', 'jeune qui fait son jogging'] };
const FIG_GEN = { F: ['mamma', 'signora', 'nonna', 'cassiera', 'cuoca', 'venditrice', 'étudiante', 'infirmière', 'impiegata', 'femme', 'maman', 'dame', 'grand-mère', 'caissière', 'cuisinière', 'vendeuse', 'étudiante', 'infirmière', 'employée', 'femme'], M: ['garçon', 'studente ', 'giovane che', 'maçon', 'tiratore', 'gardien', 'chauffeur de taxi', 'operaio', 'allenatore', 'pastore', 'barman', 'cameriere', 'venditore', 'capo', 'grossista', 'agente', 'garçon', 'étudiant ', 'jeune qui', 'maçon', 'tireur', 'gardien', 'chauffeur', 'ouvrier', 'entraîneur', 'pasteur', 'barman', 'serveur', 'vendeur', 'chef', 'grossiste', 'agent'] };
...
const RUOLO_GUIDA = { capo: 'tu es LE CHEF : tu parles de services, d\'objectifs, de clients, de discipline ; tu juges la performance et la ponctualité ; tu peux récompenser ou punir ; tu n\'es pas un ami même si tu peux être cordial', collega: 'tu es un COLLÈGUE : tu parles du travail quotidien, du chef, des horaires, des paies, des ragots de la boîte ; tu peux t\'allier au joueur ou lui faire concurrence', amico: 'tu es un AMI : tu parles de la vie, de la famille, d\'argent, de filles/garçons, de foot, d\'église ; tu peux aider mais tu as aussi tes problèmes', vicino: 'tu es le VOISIN : tu parles du quartier, du bruit, de l\'eau de la pompe, du fokontany, des voleurs ; tu observes tout ce que fait le joueur', venditore: 'tu es un VENDEUR : tu vends, tu marchandes, tu te plains de la concurrence et du prix du riz ; tu es gentil avec ceux qui achètent souvent', poliziotto: 'tu es un AGENT : formel, méfiant, tu demandes les papiers ; tu peux fermer les yeux ou non', medico: 'tu es MÉDECIN : tu poses des questions sur les symptômes, tu donnes des conseils de santé, tu parles du coût des soins', professore: 'tu es PROFESSEUR : tu parles d\'études, d\'examens, de discipline ; tu encourages ceux qui s\'appliquent', bancario: 'tu es EMPLOYÉ DE BANQUE : tu parles de comptes, de prêts, de garanties ; tu es formel', dipendente: 'tu es l\'EMPLOYÉ du joueur : tu parles du travail, tu réclames ta paie, tu te plains ou tu t\'appliques selon comment il te traite', sconosciuto: 'tu es une personne que le joueur vient de rencontrer : sois naturel, parle de ta journée et de ton métier, demande qui il est' };
const dominio = p => p.mest ? `Ton métier/rôle précis ici : ${p.mest}${p.luogoNome ? ' (' + p.luogoNome + ')' : ''}. Parle et agis selon ce que tu es : de tes clients, prix, collègues, horaires, de ce que tu vois ici.` : '';
...
function conflittoLavoretto(j) { const t = turnoOggi(); if (!t) return null; const ji = j.inizio * 60, jf = ji + j.ore * 60; const ora_ = minutiOra();
  if (ji < t.fin && jf > t.ini) return { tipo: 'sovrapposto', txt: `chevauche ton service ${hh(t.l.inizio)}–${hh(t.l.fine)}` };
  if (ji >= t.fin) return { tipo: 'dopo', txt: `après le service (tu finis à ${hh(t.l.fine)})`, primaTurno: ora_ < t.fin };
  return { tipo: 'prima', txt: `avant le service (il commence à ${hh(t.l.inizio)})` }; }
...
function schedaNota(p) {
  ...
  righe.push(`<b>Nom :</b> ${p.noto.nome ? p.nome + ' ' + p.cognome : '?'}`); righe.push(`<b>Âge :</b> ${p.noto.eta ? p.eta + ' ans' : 'environ ' + etaTxt(p)}`);
...
function iniziativaPNG() {
  ...
  if (!haCasa() && p.aff >= 50 && !S.ospite && Math.random() < 0.5) msgs.push({ t: `${p.nome}: « J'ai appris que tu dors dans la rue… Viens chez moi quelques nuits, il y a une natte. Passe à ${p.quartiere} et on en parle. »`, act: () => { p.pendOspita = true; } });
  if (S.bis.fame < 25 && p.aff >= 40) msgs.push({ t: `${p.nome}: « Tu as une de ces têtes… tu as mangé aujourd'hui ? Passe chez moi, il y a du riz. »` });
  if (p.debito && Math.random() < 0.6) msgs.push({ t: `${p.nome}: « Namako, ces ${Ar(p.debito)} Ar… tu me les rends quand ? »`, neg: 1 });
  if (p.ruolo === 'capo' && S.lavoro !== 'nulla') { if (S.perf < 40) msgs.push({ t: `${p.nome} (chef) : « Il faut qu'on parle : ça ne va pas. Viens au bureau. »`, neg: 1 }); else if (S.perf > 85 && Math.random() < 0.3) msgs.push({ t: `${p.nome} (chef) : « Beau travail ces derniers temps. Passe me voir, j'ai une proposition. »` }); }
  if (p.ruolo === 'collega' && S.lavoro !== 'nulla' && Math.random() < 0.3) msgs.push({ t: `${p.nome} (collègue) : « ${pick(['Le chef est nerveux aujourd\'hui, arrive à l\'heure.', 'On déjeune ensemble ? Hotely derrière l\'usine.', 'J\'ai entendu qu\'ils coupent les heures sup…'])}"` });
  if (p.rom >= 30 && !S.partner && Math.random() < 0.4) msgs.push({ t: `${p.nome}: « Je pensais à toi… on se voit au lac Anosy ce soir ? »` });
...
const A = {
  ...
  candidati(id) {
    ...
    if (S.flags.dritta && S.flags.dritta.id === id) { prob += 0.15; fattori.push({ n: 'Recommandation d\'une connaissance', v: 0.15 }); S.flags.dritta = null; }
  ...
  lavoretto(idx, opz) {
    ...
    if (cf && cf.tipo === 'sovrapposto') return toast(`⛔ Ce petit boulot (${hh(j.inizio)}–${hh((j.inizio + j.ore) % 24)}) ${cf.txt}. Tu ne peux pas faire les deux.`, 'bad');
    if (cf && cf.tipo === 'dopo' && cf.primaTurno) return toast(`⏰ Le petit boulot commence à ${hh(j.inizio)}, après ton service. Va d'abord travailler (${orarioLavoro(lavoro())}), puis reviens ici : l'offre reste valable jusqu'à ${hh(j.inizio + 2)}.`);
    if (cf && cf.tipo === 'prima') { const fineL = j.inizio * 60 + j.ore * 60; if (fineL > turnoOggi().ini - 20) return toast(`⛔ Tu finirais à ${hh((j.inizio + j.ore) % 24)}, trop juste avant le service de ${hh(lavoro().inizio)}.`, 'bad'); }
    if (h < j.inizio) { const t = turnoOggi(); if (t && t.fin > minutiOra() && t.ini < j.inizio * 60) return toast(`⏰ Avant le petit boulot, tu as ton service à ${hh(lavoro().inizio)} : impossible d'attendre jusqu'à ${hh(j.inizio)}.`); avanza((j.inizio - h) * 60 - now().getMinutes()); log(`Tu as attendu le début du petit boulot (${hh(j.inizio)}).`); }
    if (l.bucato) {
...
/* ---------- AI / DIALOGHI ---------- */
function ruoloTxt(p) { const map = { amico: 'ami(e)', vicino: 'voisin(e)', venditore: 'vendeur au marché', poliziotto: 'agent de police', medico: 'médecin de l\'hôpital HJRA', sconosciuto: 'connaissance', collega: 'collègue de travail', capo: 'patron au travail', professore: 'professeur(e)', 'compagno di scuola': 'camarade de classe', bancario: 'employé(e) de banque', dipendente: 'ton employé(e)' }; return (p.ruolo === 'sconosciuto' && p.mest) ? p.mest : (map[p.ruolo] || p.ruolo); }
function ospiteDi() { return S.ospite && png(S.ospite.id); }
function relazioniTxt(p) { const altri = S.png.filter(x => x.id !== p.id && x.aff >= 45).slice(0, 4).map(x => `${x.nome} (${ruoloTxt(x)}, affinité ${x.aff})`); return altri.length ? altri.join(', ') : 'personne de notable'; }
function memoriaTxt(p) { const m = (p.mem || []).slice(-6); return m.length ? m.join(' | ') : 'rien de particulier'; }
function vociTxt(p) { const v = (S.voci || []).filter(x => x.su !== p.id).slice(-4).map(x => `${x.da}: "${x.txt}"`); return v.length ? v.join(' | ') : 'aucun'; }
function azioneGiocatore(p, testo) {
  ...
  if (n > 0 && /d[oò]|dare|pago|pagare|offro|regal|prest|restitu|rendo|donne|paie|offre|prête|rends|rembourse/.test(t)) {
    if (S.soldi < n) { nota = `N'A PAS ASSEZ D'ARGENT : n'a que ${Ar(S.soldi)} Ar, l'azione fallisce`; eff.push(`Tu n'as pas ${Ar(n)} Ar (tu as ${Ar(S.soldi)} Ar).`); }
    else { const m = -trasferisci(p, -n, 'dal giocatore'); if (p.debito) { const r = Math.min(p.debito, m); p.debito -= r; if (!p.debito) { p.aff = clamp(p.aff + 6); ricorda(p, 'm\'a rendu l\'argent, personne sérieuse'); } else ricorda(p, `m'a rendu ${Ar(r)} Ar, ne mancano ${Ar(p.debito)}`); } else if (p.richiesta && m >= p.richiesta) { p.richiesta = 0; p.aff = clamp(p.aff + 4); ricorda(p, `m'a donné les ${Ar(m)} Ar che avevo chiesto`); } else { p.aff = clamp(p.aff + Math.min(8, Math.round(m / 5000))); p.favoriRicevuti = (p.favoriRicevuti || 0) + 1; ricorda(p, `m'a donné ${Ar(m)} Ar`); }
      nota = `A VRAIMENT REMIS ${Ar(m)} Ar : tu les as maintenant en poche`; eff.push(`💸 Tu as donné ${Ar(m)} Ar à ${p.nome}.`); S.eco.spesaGiocatore += m; }
  }
  else if (n > 0 && /chied|chiedo|presta(mi)?|puoi darmi|demande|prête-moi|peux-tu me donner/.test(t)) nota = `IL/ELLE TE DEMANDE ${Ar(n)} Ar: decidi tu in base ai tuoi soldi e all'affinità (usa PRESTA:n o REGALA:n solo se vuoi davvero)`;
  if (/picchi|schiaff|pugno|colpisc|spingo|aggred|frappe|gifle|coup de poing|bouscule/.test(t)) { p.aff = clamp(p.aff - 30); ricorda(p, 'M\'A AGRESSÉ(E) physiquement'); S.bis.umore = clamp(S.bis.umore - 5); nota = 'IL/ELLE T\'A AGRESSÉ(E) PHYSIQUEMENT'; eff.push(`⚠️ Tu as agressé ${p.nome}: affinità -30, rischio denuncia.`); if (Math.random() < 0.5) { S.fedina = (S.fedina || 0) + 1; const multa = Math.min(S.soldi, 50000); S.soldi -= multa; eff.push(`🚔 Denunciato/a: multa ${Ar(multa)} Ar, fedina sporca.`); } spargiVoce(p, 'è violento/a, attenzione'); }
  else if (/rub|frego|scipp|vole|pique/.test(t)) { if (Math.random() < 0.4 && p.eco && p.eco.soldi > 0) { const m = Math.min(p.eco.soldi, rnd(2000, 20000)); trasferisci(p, m, 'furto subito'); nota = `TU LUI AS VOLÉ ${Ar(m)} Ar sans qu'il/elle s'en aperçoive (il/elle ne le sait PAS)`; eff.push(`🕵️ Tu as volé ${Ar(m)} Ar à ${p.nome}.`); } else { p.aff = clamp(p.aff - 40); ricorda(p, 'A ESSAYÉ DE ME VOLER'); S.fedina = (S.fedina || 0) + 1; nota = 'TU L\'AS SURPRIS(E) EN TRAIN D\'ESSAYER DE TE VOLER'; eff.push(`🚔 Beccato/a a rubare a ${p.nome}: affinità -40, fedina sporca.`); spargiVoce(p, 'è un ladro/una ladra'); } }
  else if (/abbracc|bacio|bacia|mano nella|enlace|embrasse|bise/.test(t)) { if (p.aff >= 45 || (p.rom || 0) >= 30) { p.rom = clamp((p.rom || 0) + 6); p.aff = clamp(p.aff + 2); nota = 'gesto affettuoso gradito'; } else { p.aff = clamp(p.aff - 8); nota = 'GESTE PHYSIQUE NON APPRÉCIÉ : trop tôt, il/elle se retire'; } }
  else if (/regalo|regal|offro (un|una|del|il)|offre (un|une|du)/.test(t) && !n) { const c = /gioiell|bijou|telefono|téléphone/.test(t) ? 200000 : /fiori|fleurs|birra|bière|caff|pasto|repas|brochette|mofo|riso|riz/.test(t) ? 5000 : 15000; if (S.soldi >= c) { paga(c); movEco(p, Math.round(c * 0.3), 'regalo ricevuto (valore rivendibile)'); p.aff = clamp(p.aff + (c >= 200000 ? 20 : c >= 15000 ? 8 : 4)); nota = `cadeau vraiment remis (il t'a coûté ${Ar(c)} Ar)`; eff.push(`🎁 Regalo a ${p.nome}: -${Ar(c)} Ar.`); } else { nota = 'VOULAIT FAIRE UN CADEAU MAIS N\'A PAS L\'ARGENT : la honte'; eff.push('Tu n\'as pas l\'argent pour ce cadeau.'); } }
  else if (/cv|curriculum|lavoro|assum|travail|embauch/.test(t) && S.lavoro === 'nulla') nota = 'il/elle te demande du travail : si tu es un employeur et qu\'il/elle te convainc (politesse, expérience, besoin), tu peux utiliser ASSUMI';
  else if (/insult|vaffa|stupid|idiot|merd|con(nard)?\b|imbécile/.test(t)) { p.aff = clamp(p.aff - 12); nota = 'IL/ELLE T\'A INSULTÉ(E)'; }
...
function sistemaPrompt(p) {
  ...
  const stato = [];
  if (!haCasa()) stato.push('SANS LOGEMENT (dort dans la rue ou en auberge)'); else stato.push(`logement : ${c.nome} (confort ${c.comfort}/10)`);
  if (S.ospite) stato.push(`hébergé(e) temporairement chez ${S.ospite.nome} (encore ${S.ospite.gg} nuits)`);
  if (S.bis.fame < 30) stato.push('a visiblement FAIM'); if (S.bis.igiene < 30) stato.push('est visiblement SALE et sent mauvais'); if (S.bis.energia < 25) stato.push('semble ÉPUISÉ(E)'); if (S.bis.umore < 30) stato.push('a l\'air triste/abattu(e)');
  if (S.malattie.length) stato.push('malade : ' + S.malattie.map(m => MALATTIE[m.tipo].nome).join(', '));
  if (S.soldi < 3000) stato.push('est fauché(e) (ça se voit : demande les prix, compte les pièces)'); else if (S.soldi > 2000000) stato.push('semble aisé(e)');
  if (S.fedina) stato.push(`a un casier judiciaire (${S.fedina})`);
  if (notte) stato.push('il fait NUIT');
...
function spargiVoce(da, txt, segno) { S.voci = S.voci || []; S.voci.push({ da: da.nome, su: da.id, txt, d: S.stat.gg }); if (S.voci.length > 20) S.voci.shift(); S.png.forEach(x => { if (x.id === da.id) return; if (Math.random() < 0.6) { x.aff = clamp(x.aff + segno * rnd(1, 4)); ricorda(x, `${da.nome} m'a dit : « ${txt}"`); } }); }
...
function cardPersoneQui(list, titolo) {
  ...
  return `<div class="card"><h3>👀 ${titolo}</h3><p class="mut">Tu ne vois que l'apparence. Touche quelqu'un pour lui parler : demander un renseignement suffit pour qu'il/elle reste dans ta liste Personnes.</p>${list.map(p => `<div class="person" onclick="apriChat('${p.id}')">${avatarHtml(p)}<div class="n"><b>${nomeNoto(p)}</b><div class="mut">${attivitaTxt(p) || (p.fissa ? p.mest : doveSta(p)?.att || '')}${S.png.includes(p) ? ' · <span style="color:var(--good)">connu(e)</span>' : ''}</div></div><span class="mut">${S.png.includes(p) ? p.aff : '💬'}</span></div>`).join('')}</div>`;
}
function vLuogo(q) {
...
function vMe() {
  ...
  h += musCard();
  h += `<div class="card"><h3>⚙️ Paramètres IA</h3><p class="mut">Fournisseur actif : <b id="aiStato">…</b>. Conseillé : clé Anthropic (Claude) pour les meilleurs dialogues ; Gemini/Groq gratuits en secours.</p><select id="sProv"><option value="auto">Automatique (Claude si présent, puis les autres)</option><option value="gemini">Gemini</option><option value="groq">Groq</option><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option></select><input id="sGemini" placeholder="Clé API Google Gemini (AIza…) — gratuite"><input id="sGroq" placeholder="Clé API Groq (gsk_…) — gratuite"><input id="sOpenai" placeholder="Clé API OpenAI (sk-…)"><input id="sAnth" placeholder="Clé API Anthropic (sk-ant-…)"><input id="sModel" placeholder="Modèle (optionnel, ex. gpt-4o-mini)"><input id="sAdmin" type="password" placeholder="Clé administrateur (seulement si le serveur en ligne la demande)">${btn('Enregistrer les réglages', salvaImpostazioni, 'sec')}</div>`;
...
async function caricaImpostazioni() { try { const j = await (await fetch('/api/settings')).json(); if ($('aiStato')) { $('aiStato').textContent = j.active === 'none' ? 'moteur interne (aucune clé)' : j.active; $('sProv').value = j.provider; $('sModel').value = j.model || ''; if (j.hasGemini) $('sGemini').value = '••••'; if (j.hasGroq) $('sGroq').value = '••••'; if (j.hasOpenai) $('sOpenai').value = '••••'; if (j.hasAnthropic) $('sAnth').value = '••••'; } } catch (e) { if ($('aiStato')) $('aiStato').textContent = 'moteur interne (aucun serveur)'; } }
...
function renderModal() {
  ...
  if (M.tipo === 'chat') {
    ...
    let azioni = qui ? act('☕ Sortir ensemble', () => A.uscita(p.id), '3h · 15.000 Ar · affinité +8') + act('🎁 Cadeau : fleurs', () => A.regalo(p.id, 'fiori'), '30 min · 5.000 Ar · affinité +5') + act('🎁 Cadeau : bijou', () => A.regalo(p.id, 'gioiello'), '30 min · 200.000 Ar · affinité +20') : tel ? act('☕ Inviter à sortir (par téléphone)', () => A.uscita(p.id), '3h · 15.000 Ar · affinità +8 · vi vedete a ' + Q(S.q).nome) : '';
    if (p.debito) azioni = act(`💵 Rendre ${Ar(p.debito)} Ar`, () => A.restituisci(p.id), 'affinité +6') + azioni;
    ...
    ${ui.canale === 'no' ? '<p class="mut">Il/elle n\'est pas là : rejoins-le/la, ou appelle/écris si tu as son numéro et un smartphone.</p>' : '<div style="display:flex;gap:6px"><input id="chatIn" placeholder="' + (ui.canale === 'tel' ? 'Message ou appel…' : 'Écris quelque chose…') + '" autocomplete="off"><button class="sec" id="chatSend">➤</button></div>'}

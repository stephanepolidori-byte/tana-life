...
function log(t, tipo = '') { S.log.unshift({ t, tipo, d: dataStr() }); if (S.log.length > 100) S.log.pop(); }
function toast(t) { clearTimeout(ui.toastT); let e = document.querySelector('.toast'); if (!e) { e = document.createElement('div'); e.className = 'toast'; document.body.appendChild(e); } e.textContent = t; e.style.display = 'block'; ui.toastT = setTimeout(() => e.style.display = 'none', 2600); }
function skillUp(k, n) { S.skill[k] = clamp(S.skill[k] + n); }

/* ---------- AZIONI ---------- */
const A = {
  pronto(negozio, i) { const c = PRONTO[negozio][i]; const pr = prezzoPronto(c); if (c.mattina && ora() > 10) return toast('I mofo gasy si vendono solo la mattina.'); if (!paga(pr)) return toast('Non hai abbastanza soldi.'); S.bis.fame = clamp(S.bis.fame + c.fame); S.bis.umore = clamp(S.bis.umore + (c.umore || 3)); S.bis.salute = clamp(S.bis.salute + (c.salute || 0)); S.stat.pasti++; avanza(20); log(`Hai mangiato ${c.nome} (${Ar(pr)} Ar).`); if (c.rischio && Math.random() < c.rischio && !S.malattie.some(m => m.tipo === 'diarrea')) { S.malattie.push({ tipo: 'diarrea', gg: 3 }); log(`Il cibo non era fresco… Diarrea. MALUS: ${malusMalattia('diarrea')}`, 'bad'); } },
  ...
  cucina(rid) {
    const r = RICETTE.find(x => x.id === rid); if (!haCasa()) return toast('Serve una casa per cucinare.'); if (S.loc !== 'casa') return toast('Devi essere a casa.');
    ...
    log(`Hai cucinato ${r.nome}${fuoco ? ' (' + NOMI_DISP[fuoco].split(' ')[0] + ')' : ''}: fame +${r.fame}.`);
  },
  ...
  lavoretto(idx, opz) {
    ...
    if (h < j.inizio) { avanza((j.inizio - h) * 60 - now().getMinutes()); log(`Hai aspettato l'inizio del lavoretto (${hh(j.inizio)}).`); }
    if (l.bucato) {
...
/* ---------- AI / DIALOGHI ---------- */
function ruoloTxt(p) { const map = { amico: 'amico/a', vicino: 'vicino/a di casa', venditore: 'venditore al mercato', poliziotto: 'agente di polizia', medico: 'medico dell\'ospedale HJRA', sconosciuto: 'conoscente', collega: 'collega di lavoro', capo: 'capo al lavoro', professore: 'professore/professoressa', 'compagno di scuola': 'compagno/a di scuola', bancario: 'impiegato/a di banca', dipendente: 'tuo/a dipendente' }; return map[p.ruolo] || p.ruolo; }
function sistemaPrompt(p) {
  const l = lavoro();
  return `Sei ${p.nome} ${p.cognome}, ${p.eta} anni, ${p.gen === 'M' ? 'uomo' : 'donna'}, vivi a ${p.quartiere}, Antananarivo (Madagascar). Ruolo verso il giocatore: ${ruoloTxt(p)}${p.stato !== 'conoscente' ? ' e ' + STATO_LBL[p.stato] : ''}. Carattere: ${trattiTxt(p)}. Affinità con il giocatore: ${p.aff}/100${p.rom ? ', attrazione ' + p.rom + '/100' : ''}.
Il giocatore è ${S.nome}, ${S.eta} anni, ${S.gen === 'M' ? 'uomo' : 'donna'}, lavoro: ${l.nome}${l.id !== 'nulla' ? ' (' + orarioLavoro(l) + ', ' + giorniTxt(l) + ')' : ''}, casa: ${casa().nome}, titolo: ${EDU[S.edu]}, soldi in tasca ${Ar(S.soldi)} Ar${S.sposato ? ', sposato/a' : S.partner ? ', fidanzato/a' : ''}${S.figli.length ? ', ' + S.figli.length + ' figli' : ''}. Si trova ora a ${Q(S.q).nome}. Umore ${Math.round(S.bis.umore)}/100, igiene ${Math.round(S.bis.igiene)}/100${S.malattie.length ? ', malato/a di ' + S.malattie.map(m => MALATTIE[m.tipo].nome).join(', ') : ''}. Ora: ${dataStr()}.
Prezzi reali del gioco: riso 700 Ar/kapoaka al mercato (800 all'épicerie), zebù 3.500 Ar/250g, pomodori 500 Ar/toko, olio 1.000 Ar/sachet, mofo gasy 200 Ar nei quartieri popolari / 300 nei medi / 500 a Ivandry, vary sy laoka piccolo 2.500 Ar, completo 3.500-5.500 (fino a 14-20.000 negli hotely chic di Ankorondrano/Ivandry), taxi-be 600 Ar, stanza a Isotry 60.000 Ar/mese, monolocale 67 Ha 100-150.000, appartamento Ankorondrano ~1.300.000, villa Ivandry 4.500.000.
Regole: rispondi SEMPRE in italiano (rappresenta il malgascio parlato nella vita reale; puoi inserire raramente parole malgasce come "salama", "misaotra", "mora mora", "vazaha", "azafady"). Sii estremamente realistico e coerente con la vita quotidiana di Antananarivo (taxi-be, JIRAMA, riso, kapoaka, prezzi in Ariary, famiglia, chiesa, fady, fihavanana). Resta nel personaggio, non sei un assistente. Risposte brevi: 1-3 frasi, parlato naturale. Puoi rifiutare, arrabbiarti, chiedere favori, fare pettegolezzi, dare informazioni su lavoro/case/prezzi/quartieri. Non offrire mai soldi gratis grandi. Alla fine aggiungi su una riga il tag [AFF:n] con n tra -3 e +3 in base a quanto ti è piaciuta la frase del giocatore.`;
}
...
function renderIntro() {
  ...
  $('main').innerHTML = `<div class="hero"><div class="brand"><span>🇲🇬</span><h1>Malagasy Real Life Simulator</h1><p class="mut">Una vita vera, da zero, ad Antananarivo.</p></div></div>
...
function vLuogo(q) {
  ...
  let h = `<button class="sec" onclick="S.loc=null;render()">‹ ${q.nome}</button><h2>${d.icon} ${d.nome}</h2>`;
...
function vAgenzia() {
  ...
  ['Affitti', 'Vendite', 'Terreni'].forEach(sez => { const its = lista.filter(c => sez === 'Affitti' ? c.affitto : sez === 'Vendite' ? c.prezzo && c.tipo !== 'terreno' : c.tipo === 'terreno'); if (!its.length) return; h += `<h3 style="margin-top:8px">${sez}</h3>`; its.forEach(c => h += act(c.nome, () => conferma('🏘️ ' + (c.tipo === 'terreno' ? 'Comprare il terreno?' : c.prezzo ? 'Comprare casa?' : 'Affittare?'), `<b>${c.nome}</b><br>${c.desc || ''}<br>${c.tipo === 'terreno' ? '💰 ' + Ar(c.prezzo) + ' Ar' : c.prezzo ? '💰 ' + Ar(c.prezzo) + ' Ar in un\'unica soluzione · comfort ' + c.comfort : '💰 caparra ' + Ar(c.affitto * 2) + ' Ar subito, poi ' + Ar(c.affitto) + ' Ar ogni 1° del mese · comfort ' + c.comfort}${haCasa() && c.tipo !== 'terreno' ? '<br><b>Lasci la tua casa attuale (' + casa().nome + ').</b>' : ''}`, () => A.affitta(c.id), c.tipo === 'terreno' ? 'Compra' : c.prezzo ? 'Compra' : 'Affitta'), c.tipo === 'terreno' ? `${Ar(c.prezzo)} Ar` : c.prezzo ? `ACQUISTO ${Ar(c.prezzo)} Ar · comfort ${c.comfort}` : `${Ar(c.affitto)} Ar/mese (caparra ${Ar(c.affitto * 2)}) · comfort ${c.comfort}`)); });
...
function vAuto() { let h = `<p class="mut">${S.patente ? 'Hai la patente.' : 'Senza patente puoi comprare solo la bici.'} Veicolo attuale: ${S.veicolo ? VEICOLI.find(v => v.id === S.veicolo).nome : 'nessuno'}</p>`; VEICOLI.forEach(v => h += act(`${v.icon} ${v.nome}`, () => conferma('🚗 Comprare il veicolo?', `<b>${v.nome}</b><br>💰 ${Ar(v.prezzo)} Ar · carburante ${Ar(v.costo)} Ar ogni 4 km${v.id !== 'bici' ? '<br>Serve la patente.' : ''}`, () => A.veicolo(v.id), 'Compra'), `${Ar(v.prezzo)} Ar · ${Ar(v.costo)} Ar/4 km`, S.veicoli.includes(v.id))); if (!S.patente) h += act('🚗 Scuola guida + esame patente', () => conferma('🚗 Scuola guida?', '3h e <b>250.000 Ar</b>. L\'esame può andare male (i soldi non tornano).', A.patente, 'Vai'), '3h · 250.000 Ar'); return h; }
...
function vCitta() {
  ...
  [...QUARTIERI].sort((a, b) => distKm(S.q, a.id) - distKm(S.q, b.id)).forEach(q => { if (q.id === S.q) return; const tag = [haCasa() && casaQ() === q.id ? '🏠 casa' : '', S.lavoro !== 'nulla' && lavoro().luogo === q.id ? '💼 lavoro' : ''].filter(Boolean).join(' '); h += act(`${q.nome} <span class="mut">${['', '·', '··', '···', '····', '·····'][q.tier]}</span> ${tag}`, () => { ui.modal = { tipo: 'vai', id: q.id }; }, `${distKm(S.q, q.id)} km · ${q.poi.map(p => POI[p].icon).join('')}`); });
  return h;
}
function vPersone() {
  let h = '<h2>👥 Persone</h2>';
...
async function caricaImpostazioni() { try { const j = await (await fetch('/api/settings')).json(); if ($('aiStato')) { $('aiStato').textContent = j.active === 'none' ? 'motore interno (nessuna chiave)' : j.active; $('sProv').value = j.provider; $('sModel').value = j.model || ''; if (j.hasOpenai) $('sOpenai').value = '••••'; if (j.hasAnthropic) $('sAnth').value = '••••'; } } catch (e) { if ($('aiStato')) $('aiStato').textContent = 'motore interno (nessun server)'; } }
...
function renderModal() {
  ...
  if (M.tipo === 'vai') {
    ...
    e.innerHTML = `<div class="box"><h3>📍 ${q.nome}</h3><p class="mut">${q.desc}</p><p class="mut">${Q(S.q).nome} → ${q.nome} · ${distKm(S.q, M.id)} km · ora ${oraStr()}${inTraffico('taxibe') ? ' · 🚦 ora di punta: mezzi +70%' : ''}</p>${extra}${riga('🚶', 'A piedi', 'piedi')}${riga('🚌', 'Taxi-be', 'taxibe')}${riga('🚕', 'Taxi', 'taxi')}${v ? riga(v.icon, v.nome, 'proprio') : ''}<p class="mut" style="margin-top:8px">Luoghi: ${q.poi.map(p => POI[p].icon + ' ' + POI[p].nome.split(' (')[0]).join(' · ')}</p></div>`;
    return;
  }
  if (M.tipo === 'bucato') {
  ...
  if (M.tipo === 'morte') { e.innerHTML = `<div class="box"><h3>☠️ Sei morto/a</h3><p>La tua vita ad Antananarivo si è conclusa a ${S.eta} anni dopo ${S.stat.gg} giorni. Patrimonio finale: ${Ar(S.soldi + (S.banca?.saldo || 0))} Ar.</p>${btn('Nuova vita', () => { S = null; LS.del('tanalife'); ui.modal = null; })}</div>`; return; }
  if (M.tipo === 'chat') {
    ...
    let azioni = act('☕ Esci insieme', () => A.uscita(p.id), '3h · 15.000 Ar · affinità +8') + act('🎁 Regalo: fiori', () => A.regalo(p.id, 'fiori'), '30 min · 5.000 Ar · affinità +5') + act('🎁 Regalo: gioiello', () => A.regalo(p.id, 'gioiello'), '30 min · 200.000 Ar · affinità +20');
    ...
    <div style="display:flex;gap:6px"><input id="chatIn" placeholder="Scrivi qualcosa…" autocomplete="off"><button class="sec" id="chatSend">➤</button></div>
